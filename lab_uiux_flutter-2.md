...
| สี | หน้าที่ |
|-----|--------|
| Primary | ทำหน้าที่เป็น "ปุ่มหลัก" (ใช้สีเด่นสุด ดึงดูดสายตาให้คนกด) |
| On Primary | ทำหน้าที่เป็น "ตัวหนังสือบนปุ่มหลัก" (มักเป็นสีขาว/ดำ เพื่อให้ตัดกับสีปุ่มและอ่านง่าย) |
| Primary Container | ทำหน้าที่เป็น "กรอบพื้นหลังสีอ่อน" (ใช้เน้นพื้นที่ใหญ่ๆ แบบไม่แย่งซีน เช่น แถบเมนู) |
| On Primary Container | ทำหน้าที่เป็น "ตัวหนังสือบนกรอบสีอ่อน" (ต้องใช้สีเข้มจัด เพื่อให้อ่านข้อความได้ชัดเจน) |
...
| คำถาม | คำตอบ |
|-------|-------|
| AI ใช้ Widget อะไรสร้าง Avatar? |CircleAvatar เป็น Widget หลักที่ใช้สร้างรูปทรงวงกลม และกำหนดขนาดด้วย radius: 32|
| AI handle กรณี avatarUrl เป็น null อย่างไร? | 1. การจัดการรูปพื้นหลัง (backgroundImage)หากมี URL: จะสร้าง NetworkImage(avatarUrl!) เพื่อโหลดภาพจากอินเทอร์เน็ต หากเป็น null: จะ                                              ส่งค่า null ให้คุณสมบัตินี้ เพื่อไม่ให้ระบบพยายามโหลดรูปภาพที่ไม่มีอยู่จริง ซึ่งช่วยป้องกันข้อผิดพลาด (Exception) จากการโหลด URL ที่ไม่ถูกต้อง                                                2. การจัดการส่วนแสดงผลตรงกลาง (child) หากเป็น null: จะสร้าง Widget Text เพื่อแสดงตัวอักษรย่อ (Initials) ที่ได้จาก                                                             ฟังก์ชัน_getInitials(name)หากมี URL: จะส่งค่า null เพื่อไม่ให้ตัวอักษรย่อไปปรากฏทับซ้อนบนรูปภาพโปรไฟล์ 3. การแสดงสีพื้นหลังสำรอง เมื่ไม่มี                                              รูปภาพแสดงผล (ค่าเป็น null) ตัว CircleAvatar จะแสดงสีพื้นหลังตามที่กำหนดไว้ใน backgroundColor: colorScheme.primaryContainerซึ่ง                                            เป็นสีโทนอ่อนตามธีม Material 3 ช่วยให้ตัวอักษรย่ออ่านง่ายและดูเข้ากับส่วนอื่นของแอปพลิเคชัน|
| AI ใช้ color จาก Theme หรือ hardcode? | ใช้สีจาก Theme (Theme.of(context).colorScheme) ทั้งหมดครับ โดยไม่มีการระบุรหัสสีแบบฮาร์ดโค้ด (เช่น Colors.blue หรือ Color(0xFF...)) ลง                                            ไปในตัว Widget โดยตรง |
| มีส่วนไหนที่ควรปรับปรุง? | 1. การรับฟังก์ชันเมื่อกดปุ่ม (Callbacks)ในโค้ดเดิม ค่า onPressed ของปุ่ม Follow และ Message ถูกเว้นว่างไว้ (() {}) หากนำไปใช้งานจริง ควรเพิ่มพารามิเตอร์                                    ประเภทVoidCallback ใน Constructor เพื่อให้ Widget อื่นที่เรียกใช้งานสามารถสั่งการทำงานเมื่อกดปุ่มได้ 2. การจัดการเมื่อรูปภาพจาก URL โหลดไม่สำเร็จ (Image Loading                          Error)การใช้ CircleAvatar(backgroundImage: NetworkImage(...)) หาก URL เสียหรือผู้ใช้ไม่มีอินเทอร์เน็ต ระบบอาจจะเกิด Exception ขึ้นบนหน้าจอ และจะไม่ยอสลับ                          ไปแสดงตัวอักษรย่อให้โดยอัตโนมัติแนวทางปรับปรุง: เปลี่ยนมาใช้ ClipRRect ร่วมกับ Image.network ที่มี errorBuilder หรือใช้แพ็กเกจยอดนิยมอย่าง                                             cached_network_image เพื่อการจัดการแคชและการโหลดรูปภาพที่ราบรื่นขึ้น3. การป้องกันข้อความล้นหน้าจอ (Text Overflow)หากชื่อผู้ใช้หรืออีเมลมีความยาวมากเกินไป ทำให้                            หน้าจัดเรียงผิดเพี้ยนหรือเกิดอาการ Text Overflow ได้ ควรเพิ่มการจำกัดบรรทัดและการตัดคำ (Ellipsis) |
...
ส่วนประกอบที่มีโครงสร้างสอดคล้องกัน
โครงสร้างหลัก (Root Layout): ทั้งสองรูปแบบใช้ Scaffold เป็นฐานหลักในการควบคุมองค์ประกอบของหน้าจอเหมือนกัน
แถบด้านบน (AppBar): โครงสร้างหลักตรงกัน โดยกำหนดปุ่มเมนูด้านซ้ายในส่วน leading, แสดงข้อความชื่อร้านในส่วน title, และจัดวางวงกลมสัญลักษณ์จำลองไว้ที่ส่วน actions ทางด้านขวา
พื้นที่จัดแสดงรายการสินค้า (Body): มีการเลือกใช้งานโครงสร้างรายการแบบเลื่อนได้ (ListView) พร้อมกับกำหนดพื้นที่ขอบเพื่อเว้นระยะห่างในการจัดวางข้อมูล
การจัดรูปเล่มภายในบัตรสินค้า (ItemCard): ใช้โครงสร้างแบบแถวแนวนอน (Row) โดยเรียงลำดับจากซ้ายไปขวา ได้แก่ วงกลมตัวอักษรนำสายตา (CircleAvatar), พื้นที่ยืดหยุ่นแสดงข้อมูลตัวอักษร (Expanded ครอบโครงสร้างแนวตั้ง Column), และปิดท้ายด้วยกล่องสัญลักษณ์จำลองทางขวาสุด ซึ่งรูปแบบนี้ช่วยจัดสรรพื้นที่และลดปัญหาตัวอักษรล้นพื้นที่ทำงานได้เป็นอย่างดี
ส่วนประกอบที่มีข้อแตกต่างเชิงโครงสร้าง
ตำแหน่งปุ่มลอยสั่งการ (Floating Action Button): ในแผนภาพของคุณมีการนำเสนอโครงสร้างโดยกำหนดเป็นพารามิเตอร์ floatingActionButton ของ Scaffold โดยตรง ซึ่งเป็นวิธีการมาตรฐานและช่วยให้การจัดการโครงสร้างระบบดูเป็นระเบียบเรียบร้อยกว่าระบบก่อนหน้าที่เลือกใช้วิธีนำปุ่มไปซ้อนพิกัดผ่าน Stack ในพื้นที่แสดงผลหลัก
แถบเมนูด้านล่างสุด (Bottom Navigation): แผนภาพของคุณเลือกใช้คลาส NavigationBar ควบคู่กับ NavigationDestination ซึ่งเป็นสถาปัตยกรรมของ Material Design 3 ในขณะที่โค้ดโครงสร้างก่อนหน้านี้ใช้คลาสรุ่นเดิมอย่าง BottomNavigationBar ร่วมกับ BottomNavigationBarItem
```
---
...
```
คำตอบ: Material 3 ใช้ระบบ Tonal Palette (ColorScheme.fromSeed) ที่รองรับ Dynamic Color (ปรับสีตามวอลเปเปอร์เครื่อง) และมีคู่สี Container / On-Container ที่ช่วยเรื่อง Contrast ให้อ่านง่าย ต่างจาก Material 2 ที่ใช้ระบบสี Primary/Secondary แบบคงที่
```

**ข้อ 2:** เมื่อแปลง Figma Design เป็น Flutter Widget พบปัญหาอะไรบ้าง และแก้ไขอย่างไร?

```
คำตอบ:  ปัญหา: ข้อความหรือรูปภาพล้นจอ (Overflow), โค้ดซ้อนลึกอ่านยาก, และระยะเว้นวรรค/ฟอนต์ไม่ตรงดีไซน์
        วิธีแก้: ครอบด้วย Expanded หรือ SingleChildScrollView, แยกโค้ดซ้ำเป็น Extract Widget, และตั้งค่าสไตล์กลางใน ThemeData
```

**ข้อ 3:** Code ที่ AI สร้างให้นั้นสมบูรณ์แค่ไหน? ต้องปรับปรุงอะไรบ้าง?

```
คำตอบ: สมบูรณ์ประมาณ 70–80% ในแง่โครงสร้าง UI หลัก แต่ต้องปรับปรุง:

        1.ใส่ State Management (เช่น Provider หรือ Riverpod)

        2.แยกข้อความ/ราคาไปเก็บใน Data Model แทนการ Hardcode

        3.คลีน Widget ซ้ำซ้อนออกเพื่อลดโหลดการ Render
```

**ข้อ 4:** ถ้าจะนำ UI ที่ออกแบบไปใช้กับ Project จริง จะปรับปรุงอะไรบ้าง?

```
คำตอบ:  1.เชื่อมต่อ API: เปลี่ยน Mock Data เป็นการดึงข้อมูลจาก Server จริง

        2.วาง Architecture: แยกโค้ด UI, Logic และ Data ออกจากกัน

        3.เพิ่ม Performance & UX: ใส่ const, ทำระบบ Cache รูปภาพ และทำหน้า Loading / Error State
```
